import Person from "./src/PersonModels/Person";
import ModifyPersonHelp from "./src/PersonModels/ModifyPersonHelp";
import * as THREE from "three";
import { OrbitControls } from "@three-ts/orbit-controls";
import * as $ from "jquery";
import TargetAcquisitor from "./src/units/TargetAcquisitor";
...
function main() {
  ...
  if (localStorage.getItem("personInfo")) {
    person.destroy();
    person.limbs = JSON.parse(localStorage.getItem("personInfo"));
    person.Builder();
  }
  modifyPersonHelp = new ModifyPersonHelp(scene, person);
  targetAcquisitor = new TargetAcquisitor(canvas, scene, camera);
  {
